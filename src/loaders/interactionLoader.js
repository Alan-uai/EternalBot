...
function validateInteraction(mod, file) {
    if (!mod.customIdPrefix) return `O manipulador em ${file} não possui a propriedade "customIdPrefix".`;
    if (typeof mod.handleInteraction !== 'function') return `O manipulador ${mod.customIdPrefix} em ${file} não possui uma função "handleInteraction".`;
    return null;
}
